...
const Icon: React.FC<IconProps> = ({ d, viewBox = "0 0 24 24" }) => (
  <svg
    width="20"
    height="20"
    viewBox={viewBox}
...
const icons = {
  zoomIn: (
    <Icon d="M11 11m-6 0a6 6 0 1 0 12 0 6 6 0 1 0-12 0 M16.243 16.243L21 21 M8 11h6 M11 8v6" />
  ),
  zoomOut: (
    <Icon d="M11 11m-6 0a6 6 0 1 0 12 0 6 6 0 1 0-12 0 M16.243 16.243L21 21 M8 11h6" />
  ),
  oneToOne: (
    <svg
      width="20"
  ...
  reset: (
    <Icon d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8 M3 3v5h5" />
  ),
  ...
  rotateLeft: (
    <Icon d="M1 4v6h6 M23 20v-6h-6 M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4-4.64 4.36A9 9 0 0 1 3.51 15" />
  ),
  rotateRight: (
    <Icon d="M23 4v6h-6 M1 20v-6h6 M3.51 9a9 9 0 0 1 14.85-3.36L23 10m-22 4 4.64 4.36A9 9 0 0 0 20.49 15" />
  ),
  flipHorizontal: (
    <svg
      width="20"
  ...
  brightnessUp: (
    ...
    >
      ...
      <line x1="2.93" y1="4.93" x2="4.34" y2="6.34" />
      <line x1="15.66" y1="17.66" x2="17.07" y2="19.07" />
      <line x1="2" y1="12" x2="4" y2="12" />
      <line x1="16" y1="12" x2="18" y2="12" />
      <line x1="2.93" y1="19.07" x2="4.34" y2="17.66" />
      <line x1="15.66" y1="6.34" x2="17.07" y2="4.93" />
...
/* ─── VIEWER COMPONENT ───────────────────────────────────────────────────── */
const Viewer: React.FC<ViewerProps> = ({ images, initialIndex, onClose }) => {
  // ── state ───────────────────────────────────────────────────────────────
  ...
  const computeInitialScale = useCallback((nw: number, nh: number): number => {
  ...
  const zoomTo = useCallback(
    (s: number) => setScale(Math.max(0.01, Math.min(100, s))),
    [],
  );
  const zoomIn = () => {
    zoomTo(scale * 1.3);
    showTooltip(`${Math.round(scale * 1.3 * 100)}%`);
  };
  const zoomOut = () => {
    zoomTo(scale * 0.7);
    showTooltip(`${Math.round(scale * 0.7 * 100)}%`);
  };
  const oneToOne = () => {
    zoomTo(1);
    showTooltip("100%");
  ...
  const STEP = 7;
  const incBrightness = () => {
    const v = Math.min(200, brightness + STEP);
    setBrightness(v);
    showTooltip(`Brightness: ${v}%`);
  };
  const decBrightness = () => {
    const v = Math.max(0, brightness - STEP);
    setBrightness(v);
    showTooltip(`Brightness: ${v}%`);
  };
  const incContrast = () => {
    const v = Math.min(200, contrast + STEP);
    setContrast(v);
    showTooltip(`Contrast: ${v}%`);
  };
  const decContrast = () => {
    const v = Math.max(0, contrast - STEP);
    setContrast(v);
    showTooltip(`Contrast: ${v}%`);
  };
  ...
  const onDblClick = () => {
    ...
    if (Math.abs(scale - 1) > 0.01) {
      ...
      zoomTo(fitScale);
      showTooltip(`${Math.round(fitScale * 100)}%`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="viewer-header">
        ...
        <button
          className="viewer-btn viewer-btn-fullscreen"
      ...
      <div className="viewer-toolbar">
        ...
        <ToolBtn title="One to one" onClick={oneToOne}>
          {icons.oneToOne}
        </ToolBtn>
        <ToolBtn title="Reset" onClick={reset}>
          {icons.reset}
        </ToolBtn>
        <ToolBtn title="Previous" onClick={prev}>
          {icons.prev}
        </ToolBtn>
        <ToolBtn
        ...
        <ToolBtn title="Next" onClick={next}>
          {icons.next}
        </ToolBtn>
        <ToolBtn title="Rotate left" onClick={rotateLeft}>
          {icons.rotateLeft}
        </ToolBtn>
        <ToolBtn title="Rotate right" onClick={rotateRight}>
          {icons.rotateRight}
        </ToolBtn>
        <ToolBtn title="Flip horizontal" onClick={() => setFlipH((f) => !f)}>
          {icons.flipHorizontal}
        </ToolBtn>
        <ToolBtn title="Flip vertical" onClick={() => setFlipV((f) => !f)}>
          {icons.flipVertical}
        </ToolBtn>
        <ToolBtn title="Increase brightness" onClick={incBrightness}>
          {icons.brightnessUp}
        </ToolBtn>
        <ToolBtn title="Decrease brightness" onClick={decBrightness}>
          {icons.brightnessDown}
        </ToolBtn>
        <ToolBtn title="Increase contrast" onClick={incContrast}>
          {icons.contrastUp}
        </ToolBtn>
        <ToolBtn title="Decrease contrast" onClick={decContrast}>
          {icons.contrastDown}
        </ToolBtn>
      </div>
...
/* ─── TOOLBAR BUTTON ─────────────────────────────────────────────────────── */
const ToolBtn: React.FC<ToolBtnProps> = ({ children, onClick, title }) => {
  return (
